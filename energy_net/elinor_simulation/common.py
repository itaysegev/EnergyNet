...
from energy_net.elinor_simulation.elinorunit import ElinorUnit
...
from energy_net.elinor_simulation.elinor_rewrad import ElinorUnitRewardFunction
...
def example_unit():
    # initialize consumer devices
        ...
        # initilaize pcsunit
        return ElinorUnit(name="test_pcsunit", consumption_params_dict=consumption_params_dict, storage_params_dict=storage_params_dict, production_params_dict=production_params_dict, agg_func= None)


def default_network_entities() -> List[NetworkEntity]:
        pcsunit = example_unit()
        return [pcsunit]

ENV_CFG_FILE = Path(__file__).parent / 'simulator_env_configs.json'


def get_env_cfgs():
    with open(ENV_CFG_FILE, 'r') as f:
        env_cfgs = json.load(f)
    env_cfgs['single_entity_simple']['reward_function'] = ElinorUnitRewardFunction
...
simulator_cfgs = {k: cfg for k, cfg in test_env_cfgs.items() if 'network_entities' not in cfg or len(cfg['network_entities']) == 1}