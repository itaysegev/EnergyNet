...
def all_plots(
    ...
    algos = [algo.upper() for algo in algos]
    ...
    for env in envs:
        ...
        for algo in algos:
            for folder_idx, exp_folder in enumerate(exp_folders):
                ...
                results[env][f"{labels[folder_idx]}-{algo}"] = 0.0
                ...
                for _, dir_ in enumerate(dirs):
                    ...
                    if len(log[key]) >= max_len:
                        last_eval.append(log[key][max_len - 1])
                    else:
                        last_eval.append(log[key][-1])
                ...
                if len(merged_results) > 0:
                    ...
                    if print_n_trials:
                        print(f"{env}-{algo}-{labels[folder_idx]}: {n_trials}")
                    ...
                    if display_median:
                        results[env][f"{algo}-{labels[folder_idx]}"] = f"{np.median(last_evals):.0f}"
                    else:
                        results[env][
                            f"{algo}-{labels[folder_idx]}"
                    ...
                    post_processed_results[env][f"{algo}-{labels[folder_idx]}"] = {
                        "timesteps": timesteps,
                        "mean": mean_,
                        "std_error": std_error,
    ...
    value_matrix = [[] for i in range(len(envs) + 1)]
    ...
    if output is not None:
        print(f"Saving to {output}.pkl")
        with open(f"{output}.pkl", "wb") as file_handler:
...
if __name__ == "__main__":
    algos = ["ppo", "sac", "a2c"]
    envs = ["energy_net-v0"]
    exp_folders = ["Models/Original_no_first"]
    all_plots(
        algos,
        envs,
        exp_folders,
